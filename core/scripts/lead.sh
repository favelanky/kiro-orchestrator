...
timeout "$MAX_TIME" kiro-cli chat --no-interactive --trust-all-tools $RESUME_FLAG \
...
Be strict on reviews. Quality > speed. Reject bad code." 2>&1 | stdbuf -oL tee -a "$LOG" || log "Lead exited (timeout or error)"
